    ...
    >
      <View
        style={[
          ...
          isDark &&
            (isAgent
              ? skeletonStyles.agentBubbleDark
              : skeletonStyles.userBubbleDark),
        ]}
      >
...
export function ActivityItem({ activity, onApprovePlan }: ActivityItemProps) {
        ...
        <View style={[styles.card, isDark && styles.cardDark]}>
          ...
          {plan.steps?.map((step, index) => (
            <View key={step.id || index} style={styles.planStep}>
              <Text
                style={[styles.stepNumber, isDark && styles.stepNumberDark]}
              >
                {index + 1}.
              </Text>
  ...
  if (activity.planApprovalRequested) {
    ...
    return (
      <View style={styles.container}>
        <View
          style={[styles.card, styles.approvalCard, isDark && styles.cardDark]}
        >
          <View style={styles.cardHeader}>
          ...
          {changeSetArtifacts.length > 0 && (
            ...
            >
              ...
              <Text style={styles.codeButtonText}>
                {showCode
                  ? "Hide Code Changes"
                  : `View Code Changes (${changeSetArtifacts.length})`}
              </Text>
            </TouchableOpacity>
          )}

          {showCode &&
            changeSetArtifacts.map((artifact, index) => (
              <View key={index} style={styles.codeContainer}>
                ...
                >
                  ...
                  >
                    <DiffHighlighter
                      code={
                        (artifact.changeSet?.gitPatch?.unidiffPatch?.slice(
                          0,
                          3000,
                        ) || "") +
                        ((artifact.changeSet?.gitPatch?.unidiffPatch?.length ||
                          0) > 3000
                          ? "\n... (truncated)"
                          : "")
                      }
